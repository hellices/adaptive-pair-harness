...
- **Exported signature change** — a changed or removed exported function/class
  method signature
...
- Node.js **22.12 or newer** for local packaging; **Node 24 LTS recommended**
...
7. Start a session with one of these:
   ...
   - Chat → `@pair /start`
   - Keyboard shortcut → `Ctrl+Alt+P` on Windows/Linux or `Cmd+Alt+P` on macOS
...
- Diagnostic evidence is sanitized before remote use; the extension does not
  forward raw diagnostic text wholesale.
- The latest local inline question is **not** forwarded back to remote chat
  providers.
- Token budgets are enforced per 10-minute window:
  - `eco`: 2 calls / 2,000 estimated input tokens
  - `balanced`: 4 calls / 6,000 estimated input tokens
  - `active`: 8 calls / 12,000 estimated input tokens
...
- Valid provider values are `local-template`, `vscode-copilot`, and
  `openai-compatible`.
- `adaptivePair.model.baseUrl` must be a valid `http` or `https` URL.
...
- evidence is limited to import changes, exported signature changes/removals,
